...
export default function FaqPageSEO() {
  ...
  useEffect(() => {
    async function fetchSeoData() {
      try {
        setLoading(true);
        const docRef = doc(db, "seo_settings", "faq");
        ...
        if (docSnap.exists()) {
          ...
          const defaultData = {
            ...
            keywords:
              "FAQ, pertanyaan umum, Al-Quran custom, sajadah custom, bantuan pelanggan",
            og_image: "",
            createdAt: new Date(),
            updatedAt: new Date(),
          };

          await setDoc(doc(db, "seo_settings", "faq"), defaultData);
